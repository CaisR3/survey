...
fun recipient(rpc: CordaRPCOps, webPort: Int) {
    ...
    if (wtx.attachments.isNotEmpty()) {
        if (wtx.outputs.isNotEmpty()) {
            ...
            try {
                require(connection.responseCode == HttpServletResponse.SC_OK) { "HTTP status code was ${connection.responseCode}" }
                require(connection.contentType == MediaType.APPLICATION_OCTET_STREAM) { "Content-Type header was ${connection.contentType}" }
                require(connection.getHeaderField(HttpHeaders.CONTENT_DISPOSITION) == "attachment; filename=\"${state.hash}.zip\"") {
                    "Content-Disposition header was ${connection.getHeaderField(HttpHeaders.CONTENT_DISPOSITION)}"
                }
